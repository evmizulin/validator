...
const validator = require('../index')({ defaultErrorMessage: 'Техническая ошибка', showErrorCode: false })
const defaultValidator = require('../index')()
...
describe('Validator', () => {
  ...
  it('should return error OBJECT_REQUIRED with default error message', () => {
    const value = {
      email: 'username@email.com',
      phone: '111'
    }
    const result = defaultValidator(value, schema, errors)
    assert.deepEqual(result.valid, false)
    assert.deepEqual(result.errors, [
      {
        code: 'OBJECT_REQUIRED',
        dataPath: [],
        message: 'Technical error (code: OBJECT_REQUIRED)',
        params: {
          key: 'name'
        },
        schemaPath: ['required', '0']
